...
const Register = () => {
  ...
  const onSubmit = async (data, form) => {
    ...
    try {
      ...
      const result = await connection.register(data);
      console.log(result);
    } catch (error) {
      console.error(error);
    }
    ...
    <section id="register" className='userForm d-flex align-items-center justify-content-center'>
      <Container className="h-100">
        <Row className="w-100 d-flex align-items-center justify-content-center">
          <Col sm={12} md={10} lg={7} xl={5} className="userForm-container d-flex justify-content-center align-items-start minH-fit" style={{padding: "30px"}}>
            <div className="form-demo w-100">
              <div>
                <h1 className="text-center mb-3 text-bold">Rejestracja</h1>
                <Form
                  onSubmit={onSubmit}
                  ...
                  render={({ handleSubmit }) => (
                    <form onSubmit={handleSubmit} className="p-fluid mt-0 w-100">
                      ...
                      />
                      <Button type="submit" label="Zarejestruj się" className="mt-2 w-100" />
                    </form>
                  )}
                />
              </div>
